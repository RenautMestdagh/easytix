        ...
        <div class="flex justify-between items-center mb-10">
            <div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                    {{ __('Update ticket type for: ') }} <span class="font-medium">{{ $event->name }}</span>
                </p>
            </div>
            <x-ui.back-to-button route="ticket-types.index" :routeParams="[$event]" text="{{ __('Back to ticket types') }}"/>
        </div>
                    ...
                    <div class="space-y-8">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                @php
                                $priceDisabled = $ticketType->is_published || $ticketType->allTickets->count()!=0;
                                @endphp
                                <x-ui.forms.group label="Price (€)" for="price_euros" error="price_euros">
                                    ...
                                    @if($priceDisabled)
                                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                            {{ __('Price cannot be changed for published, reserved or bought ticket types') }}
                                        </p>
                                    @endif
                                </x-ui.forms.group>
